...
describeWithFFmpeg("exportVideo()", () => {
  ...
  it("exports with custom CRF (lower CRF = larger file)", async () => {
    ...
    await ffmpeg.exportVideo().input(FIXTURES.videoShort).crf(18).output(out18).execute();
    await ffmpeg.exportVideo().input(FIXTURES.videoShort).crf(28).output(out28).execute();
  ...
  it("applies faststart for mp4", async () => {
    ...
    await ffmpeg.exportVideo().input(FIXTURES.videoShort).faststart(true).output(out).execute();
  ...
  describe("tryExecute()", () => {
    it("returns success result on valid input", async () => {
      ...
      const result = await ffmpeg.exportVideo().input(FIXTURES.videoShort).output(out).tryExecute();
